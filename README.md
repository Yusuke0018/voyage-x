...
#### 2. マイルストーン配置の改善 🔴 優先度：高
**現状**: 全マイルストーンが同じ高さ（top: 100px）に配置
**実装内容**:
- 重なり検出アルゴリズム
- 自動的に異なる高さに配置
- 視認性の向上
...
#### 4. GitHub Pages設定 🟡 優先度：中
**現状**: ローカルのみで動作
**実装内容**:
```yaml
# .github/workflows/deploy.yml
- mainブランチプッシュ時の自動デプロイ
- gh-pagesブランチへの静的ファイルコピー
- カスタムドメイン設定（オプション）
```
**作業見積**: 15分
...
### バグ
1. **期間バーの幅が固定** - リサイズハンドル未実装
2. **マイルストーンの重なり** - 同じ高さに配置されるため重複表示
3. **タッチ操作非対応** - スマホ/タブレットでドラッグ不可
